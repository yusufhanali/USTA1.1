...
class TransformController(Node):
    def __init__(self):
        ...
        self.controlled_frame = 'overhead_hri_camera_link'
        self.parent_of_controlled_frame = 'world'
        self.world_frame = 'world'
        
        self.parent_to_world_transform = None
        while self.parent_to_world_transform is None:
            try:
                self.parent_to_world_transform = self.tf_buffer.lookup_transform(self.world_frame, self.parent_of_controlled_frame, rclpy.time.Time())
            except Exception as e:
                self.get_logger().info(f"Waiting for transform from {self.parent_of_controlled_frame} to {self.world_frame}. Error: {e}")
                rclpy.spin_once(self, timeout_sec=1.0)
        
        self.parent_to_world_matrix = linalg_utils.transform_to_matrix(self.parent_to_world_transform)
        
        
        self.position = [0.8822, -1.489, 2.2328]  # x, y, z
        #self.relative_head_orientation = [-1.6007963, -1.5457963, 0.0868] # aerial xyz
        self.relative_orientation_quat = [-0.26430327, 0.27674969, 0.65393444, 0.65262787]  # xyzw format
    ...
    def publish_tf(self):
        current_orientation = R.from_quat(self.relative_orientation_quat).as_matrix()
        current_orientation = self.parent_to_world_matrix[:3, :3] @ current_orientation
        current_orientation = R.from_euler('XYZ', self.change_wrt_world).as_matrix() @ current_orientation
        current_orientation = self.parent_to_world_matrix[:3, :3].T @ current_orientation
        
        orientation_quat = R.from_matrix(current_orientation).as_quat()  # Convert to xyzw format
        
        t = TransformStamped()
        t.header.stamp = self.get_clock().now().to_msg()
        t.header.frame_id = self.parent_of_controlled_frame
        t.child_frame_id = self.controlled_frame
        ...
        print(f'position={self.position}, orientation={self.relative_orientation_quat}, orientation_quat={orientation_quat}', end='\r')  # Print on the same line
    ...
    while rclpy.ok():
        if sys.stdin.isatty():
            ...
            try:
                ...
                if ch == 'w':
                    self.position[0] += self.translation_step
                elif ch == 's':
                    self.position[0] -= self.translation_step
                elif ch == 'a':
                    self.position[1] += self.translation_step
                elif ch == 'd':
                    self.position[1] -= self.translation_step
                elif ch == 'q':
                    self.position[2] -= self.translation_step
                elif ch == 'e':
                    self.position[2] += self.translation_step
                elif ch == 'r':
