...
if [ $? != 0 ]; then
    ...
    tmux send-keys -t $SESSION_UR5E_BRINGUP "ros2 run robot_controller breathe_and_gazing"
    ...
    tmux send-keys -t $SESSION_UR5E_BRINGUP "ros2 launch ur_robot_driver ur5e.launch.py ur_type:=ur5e robot_ip:=192.168.1.102 kinematics_params_file:=\"/home/kovan/USTA1.1/ur5e_calibration.yaml\"" C-m
    ...
    tmux send-keys -t $SESSION_UR5E_BRINGUP "conda activate yolo-v8" C-m 
    tmux send-keys -t $SESSION_UR5E_BRINGUP "cd ~/USTA1.1/ros_workspace/" C-m
    tmux send-keys -t $SESSION_UR5E_BRINGUP "source install/setup.bash" C-m
    tmux send-keys -t $SESSION_UR5E_BRINGUP "ros2 run robot_controller head_detector"
    
    tmux split-window -v -t $SESSION_UR5E_BRINGUP
    
    tmux send-keys -t $SESSION_UR5E_BRINGUP "ros2 launch realsense2_camera rs_launch.py \
  enable_color:=true \
  enable_depth:=false \
  color_width:=1920 \
  color_height:=1080 \
