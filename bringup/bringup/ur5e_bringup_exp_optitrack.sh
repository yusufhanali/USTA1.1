...
if [ $? != 0 ]; then
    ...
    tmux send-keys -t $SESSION_UR5E_BRINGUP "ros2 run robot_controller experiment_optitrack"
    ...
    tmux send-keys -t $SESSION_UR5E_BRINGUP "cd ~/USTA1.1/ros_workspace/" C-m
    tmux send-keys -t $SESSION_UR5E_BRINGUP "source install/setup.bash" C-m
    tmux send-keys -t $SESSION_UR5E_BRINGUP "ros2 launch robot_controller realsense_launch_both.py" C-m
